...
//CACHO DE CODIGO QUE FUNCIONA!!! REESCRIBE EL CONTADOR CUANDO LLEGA AL TOPE EN LA MAQUINA DE ESTADOS

/*
void CyCx3GpifCB(CyU3PGpifEventType event,uint8_t currentState)

{

	Manejar la interrupcion de la maquina de estados
	switch (event)
	{
	case CYU3P_GPIF_EVT_CTRL_COUNTER:
	{
		CyU3PGpifInitCtrlCounter(init, T, CyFalse, CyTrue, paso);
	}
	break;
	default:break;
	}
 ...
 */
#define dirN 0xC0000000
#define dirn1 0xC0000004
#define dirn2 0xC0000008
#define dirn3 0xC000000C
#define dirn4 0xC0000010
#define dirn5 0xC0000014
#define cont  0xC0000018
...
uint16_t n[5]= {1,2,3,4,5};
uint16_t N=4;
uint16_t i=0;
uint16_t init=0;
int toggle=-1;
uint16_t T=5000;		//a 25KHz, esto representa 2,4 segundos.
uint16_t seg = 1000;
uint8_t paso = 1;
...

{
	...
	case CYU3P_GPIF_EVT_CTRL_COUNTER:
	{
		...
		//if (toggle==1)						//Si el contador esta lleno
		//{
			// T como inicializacion y tope en t_n en el Counter decrementando a traves de
		CyU3PGpifInitCtrlCounter(i*seg, T, CyFalse, CyTrue, paso);
		if (toggle==1)
		//	toggle=-toggle;
			i++;
		//	return;
		...
		/*if (toggle==-1)										//Si el contador esta vacio
		{
			if (i==4)
			{
						i=0;
			...
			else
			{
				...
				CyU3PGpifInitDataCounter(0, n[i]*seg, CyFalse, CyTrue, paso);
				toggle=-toggle;
		}*/
	}
	break;
