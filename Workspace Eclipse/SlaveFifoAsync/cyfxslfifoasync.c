...
SlFifoAppThread_Entry (
        uint32_t input)
{
	uint32_t tmp_data[4] ={0xFFFF, 0x00, 0xFFFF, 0x00};
	uint32_t tmp_cnt=0;
    ...
    for (;;)
    {
      ...
      //  if (glIsApplnActive)
        ...
        {
            apiRetStatus = CyU3PGpifWriteDataWords (0, CyFalse, 1, &(tmp_data[tmp_cnt]), 0xFF);
            if (apiRetStatus == CY_U3P_SUCCESS)
            {

            	 CyU3PDebugPrint (4, "Write Data = 0x%x\n\r", tmp_data[tmp_cnt]);

            }
            else
            {
            	 CyU3PDebugPrint (4, "Write failed with error code %d\n\r", apiRetStatus );
            ...
            /* Make sure we roll back to start of data*/
            tmp_cnt++;
