            ...
            deviceNumber, &devInterfaceData)) {
                ...
                if (SetupDiGetInterfaceDeviceDetail (hwDeviceInfo,
                    ...
                    &devInfoData)) {
                        ...
                        hFile = CreateFile ((wchar_t *)DevPath,
...
bool CCyUSBDevice::Open(UCHAR dev){
    ...
    if (CreateHandle(dev)) {
        ...
        if ((BcdUSB & BCDUSBJJMASK) == USB30MAJORVER)
        {
            if(GetInternalBosDescriptor()) // USB3.0 specific descriptor
			{
				...
				catch(char *Str)
				{
                    ...
                    MessageBox(NULL,(wchar_t *)"Please correct firmware BOS descriptor table",(wchar_t *)"Wrong BOS Descriptor",NULL);
        ...
        for (int i=0; i<Configs; i++) 
		{
			...
			{
				char *Str1 = Str; // just to ignore warning
                MessageBox(NULL,(wchar_t *)"Please correct firmware descriptor table",(wchar_t *)"Wrong Device Configuration",NULL);
				Close(); // Close the device handle.
				return false;
			}
        }
...
// Esto funca cuando se compila con MSVS, para mingw usamos fopen()
